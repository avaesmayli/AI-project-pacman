...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        ls = [(pos.argMax(), dist(pacmanPosition, pos.argMax())) for pos in livingGhostPositionDistributions]
        ghost_position, _ = min(ls, key = lambda t: t[1])

        ls = [(action, dist(Actions.getSuccessor(pacmanPosition, action), ghost_position)) for action in legal]
        ret , _ = min(ls, key = lambda t: t[1])
